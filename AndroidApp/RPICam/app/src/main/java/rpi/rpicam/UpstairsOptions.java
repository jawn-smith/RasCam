...
public class UpstairsOptions extends AppCompatActivity {
    ...
    public void streamVideo(View view){
        new startStream("Initializing Stream").execute("./stream_upstairs.sh");
    ...
    public void pauseSystem(View view){
        ...
        builder.setPositiveButton("OK", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                new ServerCommand("Pausing Camera").execute("nohup ./pause_upstairs.sh " + minutes + " > upstairs_pause.out 2> upstairs_pause.err < /dev/null &");
            }
        });
        builder.setNegativeButton("Cancel", new DialogInterface.OnClickListener() {
            @Override
    ...
    public void Reboot(){
        new ServerCommand("Rebooting").execute("./reboot_upstairs.sh");
    }

    public void shutDown(){
        new ServerCommand("Shutting Down").execute("./shutdown_upstairs.sh");
    }
    ...
    private class ServerCommand extends AsyncTask<String, String, String> {
        ...
        private ProgressDialog dialog = new ProgressDialog(UpstairsOptions.this);
    ...
    private class TakeAPhoto extends AsyncTask<String, String, String> {
        ...
        private ProgressDialog dialog = new ProgressDialog(UpstairsOptions.this);
        private ProgressDialog dialog2 = new ProgressDialog(UpstairsOptions.this);
        ...
        protected String doInBackground(String... args) {
            ...
            try{
                ...
                ce.setCommand("./take_photo_upstairs.sh");
                ce.setErrStream(System.err);
            ...
            try{
                ...
                BufferedInputStream bis = new BufferedInputStream(channelSftp.get("upstairs_user_pic.jpg"));
                //File newFile = new File(args[0]);

                //get file size
                Vector list = channelSftp.ls("upstairs_user_pic.jpg");
                ChannelSftp.LsEntry lsEntry = (ChannelSftp.LsEntry) list.firstElement();
                SftpATTRS attrs = lsEntry.getAttrs();
                double filesize=attrs.getSize();

                // read in the file
                OutputStream os = openFileOutput("upstairs_user_pic.jpg", Context.MODE_WORLD_READABLE);
    ...
    private class RecordAVideo extends AsyncTask<String, String, String> {
        ...
        protected String doInBackground(String... args) {
            ...
            try{
                ...
                ce.setCommand("./record_video_stairs.sh "+args[0]);
                ce.setErrStream(System.err);
            ...
            try{
                ...
                byte[] buffer = new byte[1024];
                BufferedInputStream bis = new BufferedInputStream(channelSftp.get("upstairs_user_vid.mp4"));
                //File newFile = new File(args[0]);

                //get file size
                Vector list = channelSftp.ls("upstairs_user_vid.mp4");
                ChannelSftp.LsEntry lsEntry = (ChannelSftp.LsEntry) list.firstElement();
                SftpATTRS attrs = lsEntry.getAttrs();
                double filesize=attrs.getSize();

                // read in the file
                OutputStream os = openFileOutput("upstairs_user_vid.mp4", Context.MODE_WORLD_READABLE);
                BufferedOutputStream bos = new BufferedOutputStream(os);
                int readCount;
                double progress=0.0;
//System.out.println("Getting: " + theLine);
    ...
    private class startStream extends AsyncTask<String, String, String> {
        ...
        protected void onPostExecute(String result) {
            ...
            //String refreshedToken = FirebaseInstanceId.getInstance().getToken();
            Intent intent = new Intent(UpstairsOptions.this,Stream.class);
            Bundle b = new Bundle();
            b.putString("http","http://YourPublicIP:692");
            intent.putExtras(b);
            startActivity(intent);
        }
